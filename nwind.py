...
st.write('#### อาคารเตี้ย(low-rise building) หมายถึงอาคารที่มีความสูงเฉลี่ยของหลังคาไม่เกิน 23 เมตร หรืออาคารที่แข็งเกร็งมาก(rigid)')
...
with inputs:
    ...
    with col1:
        H_roof = st.number_input(label='ความสูงจั่วหลังคา, $H_\mathrm{roof} \mathrm{~[m]}$', min_value=0.0, max_value=23.0, value=10.0, step=0.2)
        H = st.number_input(label='ความสูงอาคาร (ชายคา), $H \mathrm{~[m]}$', min_value=0.0, max_value=23.0, value=6.0, step=0.2)
        
        
    with col2:
        W1 = st.number_input(label='ความกว้างในแนวตั้งฉากสันหลังคา, $W_1 \mathrm{~[m]}$', min_value=0.0, value=38.0, step=0.1)
        W2 = st.number_input(label='ความกว้างในแนวขนานสันหลังคา, $W_2 \mathrm{~[m]}$', min_value=0.0, value=38.0, step=0.1)
        Ds = min(W1,W2)
        st.write(r'ความกว้างด้านแคบที่สุด, $D_s = %.2f \mathrm{~m}$'%(Ds))
        
    with col1:
        slope = np.arctan((H_roof-H)/(0.5*W1))*180.0/np.pi
...
with col1:
    ...
    st.markdown('**ประเภทของอาคารและความสำคัญ**')
    col0x, buff = st.columns([0.5,0.5])
    with col0x:
        img_show('ImportantF02.jpg')
    st.markdown('**ประเภทความสำคัญของอาคารและค่าประกอบความสำคัญของแรงลม**')
    col00x, buff = st.columns([2,2])
    with col00x:
        img_show('ImportantF01.jpg')
...
with col1:
    ...
    st.markdown('**การจำแนกกลุ่มความเร็วลมอ้างอิง**')
    col0x, buff = st.columns([0.5,0.5])
    with col0x:
        img_show('Wind01.png')
...
col1, col2 = st.columns(2)
with col1:
    st.markdown('**ตารางที่ 2-3 ค่าประกอบเนื่องจากสภาพภูมิประเทศ และ นิยามสภาพภูมิประเทศแบบ A และแบบ B**')
    col0x, buff = st.columns([0.5,0.5])
    with col0x:
        img_show('Ce01.png')
...
inputshandcal = st.container()
with inputshandcal:
    st.write('### หน่วยแรงลมสถิตเทียบเท่าที่กระทำกับอาคารต้นลม')
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        I_imf = st.number_input(label='ค่าประกอบความสำคัญของแรงลม, I', min_value=0.0, max_value=1.15, value=None, step=0.1)
        q_refv = st.number_input(label='หน่วยแรงลมอ้างอิงเนื่องจากความเร็วลม, $q \mathrm{~[kg/m^2]}$', min_value=0.0, max_value=None, value=None, step=5.0)
               
    with col2:
        C_e = st.number_input(label='ค่าประกอบเนื่องจากสภาพภูมิประเทศ, $C_e \mathrm{}$ ', min_value=None, value=None , step=0.1)
        C_pg = st.number_input(label='ค่าสัมประสิทธิ์ของหน่วยแรงลมภายนอก, $C_p C_g \mathrm{}$', min_value=None, value=None, step=0.1)
        
    with col1:
        p_t = I_imf*q_refv*C_e*C_pg
        st.markdown(r'Wind Pressure, $P = %.2f \mathrm{~m/s^2}$'%(p_t))
